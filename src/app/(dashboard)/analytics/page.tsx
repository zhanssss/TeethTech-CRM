    ...
    return (
        <article className="group rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.03)] transition hover:-translate-y-0.5 hover:border-violet-200 hover:shadow-lg hover:shadow-violet-100/40">
            ...
            <div className={`mt-2 flex items-center gap-1.5 text-xs font-medium ${positive ? 'text-emerald-600' : 'text-rose-500'}`}>
                ...
                <span className="font-normal text-slate-400">к прошлому периоду</span>
            </div>
        </article>
    );
}
...
function MaterialDistribution({ shares }: { shares: Record<string, number> }) {
    ...
    return (
        <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.03)] sm:p-6">
            <h2 className="text-sm font-bold text-slate-900">Материалы в работе</h2>
            <p className="mt-1 text-xs text-slate-400">Распределение текущих заказов по материалам</p>
            {items.length ? (
                <div className="mt-7 flex flex-col items-center gap-7 sm:flex-row xl:flex-col 2xl:flex-row">
                    <div className="relative h-44 w-44 shrink-0 rounded-full" style={{ background: `conic-gradient(${gradient})` }} role="img" aria-label="Распределение материалов">
                        <div className="absolute inset-8 flex flex-col items-center justify-center rounded-full bg-white shadow-inner">
                            ...
                            <span className="text-[11px] text-slate-400">всего</span>
                        </div>
                    </div>
            ...
            ) : <p className="mt-16 text-center text-sm text-slate-400">Данных пока нет</p>}
        </section>
    );
}
...
export default function AnalyticsPage() {
    ...
    if (isError || !data) return <ErrorState title="Аналитика недоступна" onRetry={() => void refetch()} isRetrying={isFetching}>Не удалось загрузить данные по производственной нагрузке.</ErrorState>;
    ...
    return (
        <div className="mx-auto max-w-[1600px] space-y-5 pb-6">
            <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div><h1 className="text-2xl font-bold tracking-tight text-slate-950">Аналитика</h1><p className="mt-1 text-sm text-slate-500">Обзор производительности и текущей загрузки лаборатории.</p></div>
                <button type="button" onClick={() => void refetch()} disabled={isFetching} className="inline-flex items-center justify-center gap-2 self-start rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-semibold text-slate-600 shadow-sm transition hover:border-violet-300 hover:text-violet-700 disabled:opacity-60">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className={`h-4 w-4 ${isFetching ? 'animate-spin' : ''}`}><path d="M20 12a8 8 0 1 1-2.34-5.66M20 4v6h-6" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>{isFetching ? 'Обновляем…' : 'Обновить данные'}
                </button>
            </header>

            <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <DashboardMetric title="Выполнено за месяц" value={String(data.completedThisMonth)} change={formatChange(data.completedPercentageChange, '%')} positive={data.completedPercentageChange >= 0} icon="completed" />
                <DashboardMetric title="Сейчас в работе" value={String(data.inProgressCount)} change={formatChange(data.inProgressChange)} positive={data.inProgressChange <= 0} icon="progress" />
                <DashboardMetric title="Просрочено" value={String(data.overdueCount)} change={formatChange(data.overdueChange)} positive={data.overdueChange <= 0} icon="overdue" />
                <DashboardMetric title="Средний срок" value={`${data.averageCompletionDays} дн.`} change={formatChange(data.averageDaysChange, ' дн.')} positive={data.averageDaysChange <= 0} icon="average" />
            </section>
            ...
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1.7fr)_minmax(300px,0.75fr)]">
                ...
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
                    ...
                    <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.03)] sm:p-6">
                        <div className="flex items-start justify-between"><div><h2 className="text-sm font-bold text-slate-900">Оплата заказов</h2><p className="mt-1 text-xs text-slate-400">Финансовый статус заказов</p></div><span className="text-2xl font-bold text-violet-600">{paidShare}%</span></div>
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-xl bg-emerald-50 p-3"><p className="text-[11px] text-emerald-600">Оплачено</p><p className="mt-1 text-xl font-bold text-emerald-800">{data.paidOrdersCount}</p></div><div className="rounded-xl bg-amber-50 p-3"><p className="text-[11px] text-amber-600">Ожидает</p><p className="mt-1 text-xl font-bold text-amber-800">{data.unpaidOrdersCount}</p></div></div>
                    </section>
                </div>
            </div>
        </div>
