    ...
    return (
        <div className="mx-auto w-full max-w-[1500px] space-y-5 pb-6">
            <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-violet-600">Каналы коммуникации</p>
                <h1 className="mt-1 text-2xl font-extrabold tracking-tight text-slate-950 sm:text-3xl">Интеграции</h1>
                <p className="mt-2 text-sm leading-6 text-slate-500">Подключайте внешние каналы и управляйте ими из одного места.</p></div>
                <div className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs text-slate-500"><strong className="text-slate-900">1</strong> доступно · <strong className="text-slate-900">3</strong> в разработке</div>
            </header>
            ...
            <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {channels.map((channel) => {
                    ...
                    return <article key={channel.id} className={`relative overflow-hidden rounded-2xl border bg-white p-5 transition ${active ? 'border-violet-300 shadow-lg shadow-violet-950/10 ring-1 ring-violet-100' : 'border-slate-200 hover:-translate-y-0.5 hover:border-violet-200 hover:shadow-md'} ${!available ? 'opacity-80' : ''}`}>
                        <div className="flex items-start justify-between gap-3"><span className={`flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br text-white shadow-md ${channel.color}`}>{channel.icon}</span><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${available ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}><span className={`mr-1.5 inline-block h-1.5 w-1.5 rounded-full ${available ? 'bg-emerald-500' : 'bg-slate-400'}`} />{channel.status}</span></div>
                        <h2 className="mt-5 text-base font-black text-slate-900">{channel.name}</h2><p className="mt-2 min-h-10 text-xs leading-5 text-slate-500">{channel.description}</p>
                        <button type="button" disabled={!available} onClick={() => setActiveChannel(channel.id)} className={`mt-5 w-full rounded-xl px-4 py-2.5 text-xs font-bold transition ${available ? active ? 'bg-violet-600 text-white' : 'bg-violet-50 text-violet-700 hover:bg-violet-100' : 'cursor-not-allowed bg-slate-100 text-slate-400'}`}>{available ? active ? 'Настройки открыты' : 'Настроить' : 'Будет доступно позже'}</button>
                    </article>;
                })}
            </section>

            <section className="rounded-2xl border border-slate-200/80 bg-slate-50 p-3 sm:p-4">
                <div className="mb-4 flex items-center justify-between gap-3 px-1"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-sky-600">Активный канал</p><h2 className="mt-1 text-base font-black text-slate-900">Настройка Telegram</h2></div><span className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-slate-500 shadow-sm">Администратор</span></div>
