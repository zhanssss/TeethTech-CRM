...
function allowsInventoryCounting(
	check: InventoryCheck,
	rule?: InventoryStatusRule
) {
	return (
		rule?.allowsCounting ??
		(check.statusCode === 'IN_PROGRESS' ||
			(Boolean(check.startedAt) && !check.completedAt))
	)
}

function isTerminalInventory(
	check: InventoryCheck,
	rule?: InventoryStatusRule
) {
	return (
		rule?.terminal ??
		(check.statusCode === 'COMPLETED' ||
			check.statusCode === 'CANCELLED' ||
			Boolean(check.completedAt))
	)
}
...
export default function InventoryPanel() {
	...
	const detailQuery = useGetInventoryCheckQuery(selectedId ?? '', {
		skip: !selectedId
	})
	...
	const filters = useMemo<
		Array<{ value: '' | InventoryCheckStatus; label: string }>
	>(
		() =>
	...
	const savedItemsStartIndex = useMemo(
		() =>
	...
	const checkStatusRule = check
		? statusRulesByCode.get(check.statusCode)
		: undefined
	const checkIsInitial = check
		? isInitialInventory(check, checkStatusRule)
		: false
	const checkAllowsCounting = check
		? allowsInventoryCounting(check, checkStatusRule)
		: false
	const checkIsTerminal = check
		? isTerminalInventory(check, checkStatusRule)
		: false
	const checkLocksWarehouse =
		checkStatusRule?.locksWarehouse ?? checkAllowsCounting
	const actionLoading =
		startState.isLoading || cancelState.isLoading || completeState.isLoading
	const inventoryItemsFetching = itemsQuery.isFetching
	const inventoryRowsLoading =
		inventoryItemsFetching && displayItems.length === 0
	...
	const handleCreate = async (event: FormEvent<HTMLFormElement>) => {
		...
		if (!comment.trim()) {
			setCreateError(
				'Добавьте комментарий, чтобы инвентаризацию было легко найти в истории'
			)
			return
		}
			...
			<div className="space-y-5">
							...
							<div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
								<div className="min-w-0">
									<div className="flex flex-wrap items-center gap-2">
										<span
											className={`rounded-full border px-2.5 py-1 text-xs font-bold ${getInventoryStatusClasses(check.statusCode, checkStatusRule)}`}
										>
											{getInventoryStatusLabel(
												check.statusCode,
												checkStatusRule
									...
									<p className="mt-1 text-sm text-slate-500">
										{check.startedAt
											? `Начата ${formatDateTime(check.startedAt)}`
											: 'Ещё не начата'}{' '}
										· {displayItems.length} позиций
									</p>
									{checkIsTerminal && check.completedAt && (
										<p className="mt-1 text-xs text-slate-400">
											Закрыта {formatDateTime(check.completedAt)}
										</p>
									)}
								</div>
							...
							{checkAllowsCounting && !checkIsTerminal && (
								<div className="mt-5 rounded-xl bg-slate-50 p-4">
									<div className="flex items-center justify-between gap-3 text-xs font-bold text-slate-600">
										<span>
											Пересчитано {progress.counted} из {progress.total}
										</span>
									...
									{checkLocksWarehouse && (
										<p className="mt-2 text-xs text-slate-500">
											Обычные операции списания и возврата заблокированы до
											завершения или отмены.
										</p>
									)}
								</div>
							)}
						...
						<section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
							...
							{itemsQuery.isError && displayItems.length === 0 && (
								<div className="m-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
									{getApiErrorMessage(
										itemsQuery.error,
										'Не удалось загрузить позиции инвентаризации'
									)}
								</div>
							)}
									...
									<tbody className="divide-y divide-slate-100">
										{displayItems.map((item, index) => {
											const showSavedHeader =
												index === savedItemsStartIndex &&
												savedItemsStartIndex !== -1

											const typedActualQuantity =
												checkAllowsCounting && !checkIsTerminal
													? parseQuantityInput(counts[item.id])
													: item.actualQuantity
											const hasActualQuantity =
												typedActualQuantity !== null &&
												typedActualQuantity !== undefined
											const savedCounted =
												...
												<Fragment key={item.id}>
													{showSavedHeader && (
														<tr>
															<td
																colSpan={5}
																className="bg-slate-100 px-5 py-3 text-sm font-bold uppercase tracking-wide text-slate-500"
															>
																Сохраненные
															</td>
														</tr>
													...
													<tr className="hover:bg-slate-50/70">
														...
														<td
															className={`px-5 py-4 text-sm font-black ${discrepancy > 0 ? 'text-emerald-600' : discrepancy < 0 ? 'text-red-600' : 'text-slate-400'}`}
														>
															{hasActualQuantity
																? `${discrepancy > 0 ? '+' : ''}${formatQuantity(discrepancy, item.unit)}`
																: '—'}
														</td>
														<td className="px-5 py-4 text-right">
															{checkAllowsCounting && !checkIsTerminal ? (
																...
																<span
																	className={`text-xs font-bold ${savedCounted ? 'text-emerald-600' : 'text-slate-400'}`}
																>
																	{savedCounted
																		? 'Пересчитано'
																		: 'Не пересчитано'}
				...
				{confirmation && check && (
					<Modal contentClassName="max-w-lg p-5 sm:p-6">
						...
						>
							<div
								className={`flex h-11 w-11 items-center justify-center rounded-full text-xl font-black ${confirmation === 'cancel' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-700'}`}
							>
								{confirmation === 'cancel' ? '!' : '✓'}
							</div>
			...
			<section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
				<div className="flex flex-col gap-4 border-b border-slate-100 p-5 lg:flex-row lg:items-center lg:justify-between">
					<div>
						...
						<p className="mt-1 text-sm text-slate-500">
							История, статусы и быстрый доступ к текущему пересчёту.
						</p>
					</div>
					<div className="flex flex-col gap-3 sm:flex-row">
						<select
							value={filter}
							onChange={event =>
								setFilter(event.target.value as '' | InventoryCheckStatus)
							}
							className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700 outline-none focus:border-blue-500"
						>
							{filters.map(item => (
								<option
									key={item.value}
						...
						<button
							...
							className="rounded-2xl bg-blue-600 px-5 py-3 text-sm font-bold text-white shadow transition hover:bg-blue-700"
						>
				...
				<div className="overflow-x-auto">
					<table className="w-full min-w-[760px] text-left">
						<thead className="border-b border-slate-200 bg-slate-50 text-[11px] uppercase tracking-wider text-slate-400">
							<tr>
								<th className="px-5 py-4 font-semibold">Инвентаризация</th>
								<th className="px-5 py-4 font-semibold">Начата</th>
								<th className="px-5 py-4 font-semibold">Позиций</th>
								<th className="px-5 py-4 font-semibold">Статус</th>
								<th className="px-5 py-4 text-right font-semibold">Действие</th>
							</tr>
						</thead>
						<tbody className="divide-y divide-slate-100">
							{checks.map(item => (
								...
								>
									...
									<td className="px-5 py-4">
										<span
											className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold ${getInventoryStatusClasses(item.statusCode, statusRulesByCode.get(item.statusCode))}`}
										>
											{getInventoryStatusLabel(
												item.statusCode,
												statusRulesByCode.get(item.statusCode)
											)}
										</span>
									</td>
									<td className="px-5 py-4 text-right">
				...
				{!listQuery.isLoading && checks.length === 0 && (
					<div className="px-5 py-12 text-center text-sm text-slate-500">
						Инвентаризаций с таким статусом нет
					</div>
				)}
			</section>
				...
				<Modal contentClassName="max-w-lg p-5 sm:p-6">
					<form onSubmit={handleCreate}>
						...
						<p className="mt-3 text-sm leading-6 text-slate-600">
							Будет создан снимок всех складских позиций. Одновременно может
							существовать только одна активная инвентаризация.
						</p>
